...
1. 打开 `Settings`，确认至少一个 Local CLI Agent 显示「已发现」（`discovered`），点 `Add` 添加后状态变为 `unverified`，再点 `Diagnose` 验证通过后变为 `ready`；或直接添加一个 API Assistant。
...
> **`timeout` 语义**：Agent 的 `timeout` 是**空闲超时**——只有当 agent 持续无输出时才计时，每次有新输出都会重置计时器。此外引擎有内置**硬超时**（2 小时）作为兜底，防止进程无限挂起。因此长任务只要持续有输出就不会触发空闲超时。
...
> **注意**：Turing 尚未发布到 npm（`turing` 包名在 npm 上属于其他项目）。以下示例从本地源码相对路径导入；接口实际定义在 `src/types.ts`。
...
GET    /api/agents
POST   /api/agents           # 创建 API Assistant（anthropic-api 等）
GET    /api/keys
POST   /api/keys

POST   /api/config/agents    # 添加本地 CLI Agent 配置（Settings UI 调用）
...
数据存储在 `~/.turing/turing.db`。设置环境变量 `TURING_HOME` 可覆盖默认目录（例如 `TURING_HOME=/data/turing` 会使用 `/data/turing/turing.db` 和 `/data/turing/config.json`）。消息默认保留 30 天：
